...
public class NewsService {
    ...
    private final String newsFolder = "newImages";

    // ====== CREATE =======
    public News createNews(News news, MultipartFile image) throws IOException {
        if (image != null && !image.isEmpty()) {
            String imageUrl = UploadService.storeImage(newsFolder, image);
    ...
    public News updateNews(Long id, News updatedNews, MultipartFile image) throws IOException {
        ...
        if (image != null && !image.isEmpty()) {
            // Xóa ảnh cũ nếu có
            if (exist.getThumbnail() != null) {
                UploadService.deleteImage(exist.getThumbnail().replace(UploadService.rootUrl, ""));
            }
            // Lưu ảnh mới
            String imageUrl = UploadService.storeImage(newsFolder, image);
            exist.setThumbnail(imageUrl);
        }
    ...
    private NewsDTO.NewsResponse toResponse(News n) {
        ...
        dto.setThumbnail(n.getThumbnail());
        dto.setAuthor(n.getAuthor());
        dto.setCreatedAt(n.getCreatedAt());
        dto.setLikes(countLikes(n.getId()));
        dto.setComments(commentRepo.countByNewsId(n.getId()));
